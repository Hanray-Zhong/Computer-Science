...
void ext_var_list(struct node *T)
{
    ...
    case EXT_VARDEC_LIST:
        ...
        break;
    case ID:
        rtn = fillSymbolTable(T->type_id, createAlias(), LEV, T->type, 'V',T->offset); //最后一个变量名
        if (rtn == -1)
    ...
    case ARRAY:
        ///数组定义 的关于符号表的处理
        rtn = fillSymbolTable(T->type_id, createAlias(), LEV, T->type, 'A',T->offset); //最后一个变量名
        if (rtn == -1)
...
void semantic_Analysis(struct node *T)
{
    ...
    if (T)
    {
        ...
        case EXT_DEF_FUNC:                                                                                                      //填写函数定义信息到符号表
            ...
            strcpy(T->ptr[2]->Snext,createLabel());  //函数体语句执行结束后的位置属性
            T->ptr[2]->break_num = 0;
            semantic_Analysis(T->ptr[2]); //处理函数体结点
            //计算活动记录大小,这里offset属性存放的是活动记录大小，不是偏移
            symbolTable.symbols[T->ptr[1]->place].offset=T->offset+T->ptr[2]->width;
            T->code=merge(3,genLabel(T->ptr[2]->Snext),T->ptr[1]->code,T->ptr[2]->code);          //函数体的代码作为函数的代码
            break;
        case FUNC_DEC:                                                        //根据返回类型，函数名填写符号表,，，此时是函数定义
            rtn = fillSymbolTable(T->type_id, createAlias(), LEV, T->type, 'F',0); //函数不在数据区中分配单元，偏移量为0
            if (rtn == -1)
            {
        ...
        case VAR_DEF: //处理一个局部变量定义,将第一个孩子(TYPE结点)中的类型送到第二个孩子的类型域
           ...
           // width=T->ptr[1]->type==INT?4:FLOAT?4:CHAR?1:8;  //一个变量宽度
            ...
            { //处理所有VARDEC_LIST结点
                ...
                if (T0->ptr[1])
                    T0->ptr[1]->type = T0->type; //类型属性向下传递
                T0->ptr[0]->offset=T0->offset;
                if (T0->ptr[1]) T0->ptr[1]->offset=T0->offset+width;
                if (T0->ptr[0]->nodeKind == ID)
                {
                    rtn = fillSymbolTable(T0->ptr[0]->type_id, createAlias(), LEV, T0->ptr[0]->type, 'V',T->offset+T->width); //此处偏移量未计算，暂时为0
                ...
                { ////?????数组待补
                    ...
                    T->width+=width;
                    rtn = fillSymbolTable(T0->ptr[0]->type_id, createAlias(), LEV, T0->ptr[0]->type, 'A',T->offset+T->width); //最后一个变量名
                    if (rtn == -1)
                        semantic_error(T0->ptr[0]->pos, T0->ptr[0]->type_id, "变量名重复定义");
                    else if (T0->ptr[0]->size <= 0)
                    {
                        semantic_error(T0->ptr[0]->pos, T0->ptr[0]->type_id, "数组大小不能为负值或0");
                    }
                    else
                    {
                        T0->ptr[0]->place = rtn;
                ...
                {
                    rtn = fillSymbolTable(T0->ptr[0]->ptr[0]->type_id, createAlias(), LEV, T0->ptr[0]->type, 'V',T->offset+T->width); //此处偏移量未计算，暂时为0
                    if (rtn == -1)
                    ...
                    {
                        semantic_error(T0->ptr[0]->ptr[0]->pos, T0->ptr[0]->ptr[0]->type_id, "数组大小不能为负值或0");
                    }
                    else
                    {
                        T0->ptr[0]->place = rtn;
...
void Exp(struct node *T)
{
    ...
    if (T)
    {
        ...
        case ARRAY:
            ...
            break;
        case INT:
            T->place = fill_Temp(createTemp(), LEV, T->type, 'T',T->offset); //为整常量生成一个临时变量
            T->type = INT;
            opn1.kind = INT;
            ...
            break;
        case FLOAT:
            T->place = fill_Temp(createTemp(), LEV, T->type, 'T',T->offset); //为浮点常量生成一个临时变量
            T->type = FLOAT;
            opn1.kind = FLOAT;
            ...
            break;
        case CHAR:
            T->place = fill_Temp(createTemp(), LEV, T->type, 'T',T->offset); //为字符常量生成一个临时变量
            T->type = CHAR;
            opn1.kind = CHAR;
        ...
        case ASSIGNOP:
            ...
            break;
        case AND: //按算术表达式方式计算布尔值，未写完 //其实感觉这里单指操作没有语义分析
            Exp(T->ptr[0]);
            Exp(T->ptr[1]);
            ...
            T->type = bool;
            break;
        case OR: //按算术表达式方式计算布尔值，未写完//其实感觉这里单指操作没有语义分析
            Exp(T->ptr[0]);
            Exp(T->ptr[1]);
            if (T->ptr[0]->type == INT || T->ptr[1]->type==INT){
                T->type = bool;
                ...
                semantic_error(T->pos,"", "逻辑运算符or左右类型不匹配");
                break;
            }
            T->type = bool;
            break;
        case RELOP: //按算术表达式方式计算布尔值，未写完//其实感觉这里的操作没有语义分析
            T->type = bool;
            T->ptr[0]->offset=T->ptr[1]->offset=T->offset;
            Exp(T->ptr[0]);
            Exp(T->ptr[1]);
        ...
        case SELFADD: //未写完整
            if (T->ptr[0])
            {
               /* printf("%s\n",T->type_id);
                printf("%s\n",T->ptr[0]->type_id);
                int place=fill_Temp(createTemp(),LEV,T->type,'T',T->offset); //为整常量生成一个临时变量
                //T->type=INT;
                opn1.kind=ID;
...
void boolExp(struct node *T)
{
    ...
    if (T)
    {
        ...
        case INT:
               ...
               T->width=0;
            T->type = bool; //bool表示都可以，只要是int，满足语义，只是0为假，其他为真
            break;
        case FLOAT:
              if (T->type_float!=0.0)
                    T->code=genGoto(T->Etrue);
