...
void prn_symbol()
{
    int i = 0;
    printf("+----------+----------+----------+--------------------+\n");
    printf("|variable  |layer     |type      |Scope               |\n");
    printf("+----------+----------+----------+--------------------+\n");
    //,"偏移量"
    for (i = 0; i < symbolTable.index; i++)
    {
        if(!strcmp(symbolTable.symbols[i].name,""))
            continue;
        printf("|%10s", symbolTable.symbols[i].name);
        printf("|%10d", symbolTable.symbols[i].level);
...
void ext_var_list(struct node *T)
{
    ...
    case EXT_VARDEC_LIST:
        ...
        break;
    case ID:
        rtn = fillSymbolTable(T->type_id, createAlias(), LEV, T->type, 'V', T->scope); //最后一个变量名
        if (rtn == -1)
    ...
    case ARRAY:
        ///数组定义 的关于符号表的处理
        rtn = fillSymbolTable(T->type_id, createAlias(), LEV, T->type, 'A', T->scope); //最后一个变量名
        if (rtn == -1)
...
void semantic_Analysis(struct node *T)
{
    ...
    if (T)
    {
        ...
        case FUNC_DEC:                                                        //根据返回类型，函数名填写符号表,，，此时是函数定义
            rtn = fillSymbolTable(T->type_id, createAlias(), LEV, T->type, 'F', T->scope); //函数不在数据区中分配单元，偏移量为0
        ...
        case FUNC_PARAM_LIST: //处理函数形式参数列表
            ...
            break;
        case FUNC_PARAM_DEC:
            rtn = fillSymbolTable(T->ptr[1]->type_id, createAlias(), 1, T->ptr[0]->type, 'P', T->scope);
            if (rtn == -1)
        ...
        case VAR_DEF: //处理一个局部变量定义,将第一个孩子(TYPE结点)中的类型送到第二个孩子的类型域
            ...
            { //处理所有VARDEC_LIST结点
                ...
                {
                    rtn = fillSymbolTable(T0->ptr[0]->type_id, createAlias(), LEV, T0->ptr[0]->type, 'V', T->scope); //此处偏移量未计算，暂时为0
                    if (rtn == -1)
                ...
                { 
                    rtn = fillSymbolTable(T0->ptr[0]->type_id, createAlias(), LEV, T0->ptr[0]->type, 'A', T->scope); //最后一个变量名
                    if (rtn == -1)
                ...
                {
                    rtn = fillSymbolTable(T0->ptr[0]->ptr[0]->type_id, createAlias(), LEV, T0->ptr[0]->type, 'V', T->scope); //此处偏移量未计算，暂时为0
                    if (rtn == -1)
...
void Exp(struct node *T)
{
    ...
    if (T)
    {
        ...
        case ARRAY:
            ...
            break;
        case INT:
            T->place = fill_Temp(createTemp(), LEV, T->type, 'T',T->scope); //为整常量生成一个临时变量
            T->type = INT;
            opn1.kind = INT;
            ...
            break;
        case FLOAT:
            T->place = fill_Temp(createTemp(), LEV, T->type, 'T',T->scope); //为浮点常量生成一个临时变量
            T->type = FLOAT;
            opn1.kind = FLOAT;
            ...
            break;
        case CHAR:
            T->place = fill_Temp(createTemp(), LEV, T->type, 'T',T->scope); //为字符常量生成一个临时变量
            T->type = CHAR;
            opn1.kind = CHAR;
        ...
        case PLUS: // T->ptr[0]->offset=T->offset;
            ...
            T->place = fill_Temp(createTemp(), LEV, T->type, 'T',T->scope); //,T->offset+T->ptr[0]->width+T->ptr[1]->width
            opn1.kind = ID;
        ...
        case FUNC_CALL: //根据T->type_id查出函数的定义，如果语言中增加了实验教材的read，write需要单独处理一下
            ...
            if (T->ptr[0])
            {
                ...
                T->place = fill_Temp(createTemp(), LEV, T->type, 'T',T->scope); //,T->offset+T->width-width
                opn1.kind = ID;
