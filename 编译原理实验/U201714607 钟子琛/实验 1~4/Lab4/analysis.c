...
/*void prn_symbol()
{
	...
	printf("\n│                       Var and func Table                       │");
	printf("\n├────────────┬────────────┬────────────┬────────────┬────────────┤");
    printf("\n│%12s│%12s│%12s│%12s│%12s│ \n", "var", "alias", "level", "type", "flag");
    //,"偏移量"
    for (i = 0; i < symbolTable.index; i++){
    	type=symbolTable.symbols[i].type;
    	switch(type){
    		case INT:
        		printf("│%12s│%12s│%12d│%12s│%12c│\n", symbolTable.symbols[i].name,
...
void prn_symbol()
{
    ...
    for (i = 0; i < symbolTable.index; i++)
    {
        if(symbolTable.symbols[i].type==INT)
            ...
            strcpy(type,"char");
        printf("%6s %6s %6d  %6s %6d  %c\n", symbolTable.symbols[i].name,
               symbolTable.symbols[i].alias, symbolTable.symbols[i].level,
               /*symbolTable.symbols[i].type == INT ? "int" : FLOAT ? "float" : "char"*/type,symbolTable.symbols[i].offset,symbolTable.symbols[i].flag);
               /*symbolTable.symbols[i].flag*/
    }
...
void ext_var_list(struct ASTNode *T){  //处理变量列表
    ...
    switch (T->kind){
        case EXT_DEC_LIST:
                ...
                break;
        case ID:
            rtn=fillSymbolTable(T->type_id,createAlias(),LEV,T->type,'Q',T->offset);  //最后一个变量名
            if (rtn==-1)
...
{//处理基本表达式，参考文献[2]p82的思想
  ...
  if (T)
	...
	case ID:    //查符号表，获得符号表中的位置，类型送type
                ...
                break;
    case INT:   T->place=fill_Temp(createTemp(),LEV,T->type,'T',T->offset); //为整常量生成一个临时变量
                T->type=INT;
                ...
                T->width=4;
                break;
    case FLOAT: T->place=fill_Temp(createTemp(),LEV,T->type,'T',T->offset);   //为浮点常量生成一个临时变量
                T->type=FLOAT;
    ...
    case FUNC_CALL: //根据T->type_id查出函数的定义，如果语言中增加了实验教材的read，write需要单独处理一下
                ...
                T->place=fill_Temp(createTemp(),LEV,T->type,'T',T->offset+T->width-width);
...
//                        T->width+=T->ptr[1]->width;//顺序结构顺序分配单元方式
	...
	case IF_THEN_ELSE:
                ...
                break;
	case WHILE: strcpy(T->ptr[0]->Etrue,createLabel());  //子结点继承属性的计算
                strcpy(T->ptr[0]->Efalse,T->Snext);
                T->ptr[0]->offset=T->ptr[1]->offset=T->offset;
